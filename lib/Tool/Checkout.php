	...
	function stepOrderPreview(){
		...
		$view->on('click','.xepan-checkout-proceed-to-payment',function($js,$data)use($billing_detail,$payment_step_url){
			...
			foreach ($cart_m as $cart_item) {
				...
				if(!$custom_size)
					$custom_size = $custom_field_array[0][$generic_m['id']]['custom_field_value_name'];
				
				$item_stock_m = $this->add('xepan\commerce\Model_ItemStock');
				$stock_available = $item_stock_m->getStock($cart_item['item_id'], $custom_size);
																				
				if($stock_available < $cart_item['qty'])
					return $js->univ()->errorMessage("can't proceed, ".$cart_item['name']." not in stock");
			}
