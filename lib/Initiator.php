...
class Initiator extends \Controller_Addon {
	...
	function setup_admin(){		
		...
		if(!$this->app->isAjaxOutput() && !$this->app->getConfig('hidden_xepan_commerce',false)){
			...
			// $m->addItem(['Feeds','icon'=>' fa fa-file-o'],'xepan_cms_feeds');
			// $m->addItem(['Current Opening','icon'=>'fa fa-user'],$this->app->url('xepan_commerce_currentopening'));
			// $m->addItem(['Category Header Image','icon'=>'fa fa-image'],'xepan_commerce_categoryimage');
	...
	function setup_frontend(){				
		...
		// if($country_m->loaded() AND substr($this->app->url() , 24 , 28) == "shop" AND $country_m['status'] =='InActive' ){
		// 	$this->app->redirect($this->app->url('collection',['xsnb_category_id'=>'5596']));
		// }

		// if(substr($this->app->url() , 24 , 28) == "shop"){
		// 	$this->app->redirect($this->app->url('collection',['xsnb_category_id'=>'5596']));
		// }	
